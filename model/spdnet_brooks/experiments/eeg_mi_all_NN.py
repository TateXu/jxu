...
"""
def hdm05(data_loader, save_file, model_select='1'):
    ...
    class HDMNet_6(nn.Module):
        def __init__(self):
            ...
            self.linear = nn.Linear(dim4**2, classes).to(device).double()
        ...
        def forward(self, x):
            ...
            x_spd2 = self.reeig2(self.batchnorm2(self.bimap2(x_spd1)))
            x_spd3 = self.reeig3(self.batchnorm3(self.bimap3(x_spd2)))
            x_spd4 = self.logeig4(self.reeig4(self.batchnorm4(self.bimap4(x_spd3))))
            x_vec = x_spd4.view(x_spd4.shape[0], -1)
            y = self.linear(x_vec)
            return y



    class HDMNet_7(nn.Module):
        def __init__(self):
            super(__class__, self).__init__()
            dim0 = 128
            dim1 = 10
            ...
            self.linear = nn.Linear(dim2**2, classes).to(device).double()

        def forward(self, x):

            x_spd1 = self.reeig1(self.batchnorm1(self.bimap1(self.batchnorm0(x))))
            x_spd2 = self.logeig2(self.reeig2(self.batchnorm2(self.bimap2(x_spd1))))
            x_vec = x_spd2.view(x_spd2.shape[0], -1)
            y = self.linear(x_vec)
            return y



    class HDMNet_8(nn.Module):
        def __init__(self):
            super(__class__, self).__init__()
            dim0 = 128
            dim1 = 10
...
if __name__ == "__main__":
    ...
    for model_val in ['1', '2']:
        for pval in [0.2, 0.3, 0.4, 0.5]:
            batch_size = 10 # batch size
            aa = DataLoaderMOABB(subject=2, session=0, pval=pval,
                                batch_size=batch_size)
            hdm05(aa, save_file=f'Model{model_val}_pval{str(pval)}_bs{str(batch_size)}.pkl', model_select=model_val)

        for batch_size in [5, 10, 20, 40, 80, 160, 320]:
            pval = 0.2   # test percentage
            aa = DataLoaderMOABB(subject=2, session=0, pval=pval,
                                batch_size=batch_size)
            hdm05(aa, save_file=f'Model{model_val}_pval{str(pval)}_bs{str(batch_size)}.pkl', model_select=model_val)
    for model_val in ['3', '4', '5', '6', '7', '8']:
        pval = 0.2
        batch_size = 10 # batch size
        aa = DataLoaderMOABB(subject=2, session=0, pval=pval,
                            batch_size=batch_size)
