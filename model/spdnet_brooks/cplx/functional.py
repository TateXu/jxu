...
def conv_cplx(X, conv_re, conv_im):
    if(isinstance(X, list)):
        ...
        P_Re = [conv_re(xx[0][None, ...])-conv_im(xx[1][None, ...])
                for xx in XX]
        P_Im = [conv_im(xx[0][None, ...])+conv_re(xx[1][None, ...])
                for xx in XX]
...
def batch_norm2d_cplx(X, running_mean, running_var, gamma11, gamma12, gamma22, bias, momentum, training):
    ...
    if(training):
        ...
        running_var_sqinv = utils.twobytwo_sqinv(
            running_var[:, 0, 0], running_var[:, 1, 1], running_var[:, 1, 0])
...
def cov_pool_cplx(f, reg_mode='mle', N_estimates=None):
    ...
    if(N_estimates is not None):
        ...
        if(f_re[-1].shape[-1] != self._Nestimates):
            f_re = th.cat(f_re[:-1]+(th.cat((f_re[-1], th.zeros(N,
                                                                1, n, self._Nestimates-f_re[-1].shape[-1])), -1),), 1)
        else:
            f_re = th.cat(f_re, 1)
        f_im = f_im.split(self._Nestimates, -1)
        if(f_im[-1].shape[-1] != self._Nestimates):
            f_im = th.cat(f_im[:-1]+(th.cat((f_im[-1], th.zeros(N,
                                                                1, n, self._Nestimates-f_im[-1].shape[-1])), -1),), 1)
        else:
            f_im = th.cat(f_im, 1)
