...
def init_bimap_parameter_identity(W):
    ...
    for i in range(ho):
        for j in range(hi):
            W.data[i, j] = th.eye(ni, no, device=W.device)
...
def bimap_channels(X, W):
    ...
    P = th.zeros(batch_size, channels_out, n_out,
                 n_out, dtype=X.dtype, device=X.device)
    for co in range(channels_out):
        P[:, co, :, :] = sum([bimap(X[:, ci, :, :], W[co, ci, :, :])
...
def modeig_forward(P, op, eig_mode='svd', param=None):
    ...
    U, S = th.zeros_like(P, device=P.device), th.zeros(
        batch_size, channels, n, dtype=P.dtype, device=P.device)
...
def cov_pool_mu(f, reg_mode):
    ...
    mu = f.mean(-1, True)
    f = f-mu
    X = f.matmul(f.transpose(-1, -2))/(T-1)+alpha * \
        mu.matmul(mu.transpose(-1, -2))
    aug1 = th.cat((X, alpha*mu), 2)
    aug2 = th.cat((alpha*mu.transpose(1, 2),
...
def add_id(P, alpha):
    ...
    for i in range(P.shape[0]):
        P[i] = P[i]+alpha*P[i].trace()*th.eye(P[i].shape[-1],
