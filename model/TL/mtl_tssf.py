...
comp_list = [4, 6, 10, 20, 40, 60]

for id_logvar, f_logvar in enumerate([True, False]):
    acc_mat = np.zeros((10, 11, len(comp_list)))

    for ind_comp, nr_comp in enumerate(comp_list):
        if pre_load_data:
        ...
        for nr_subj in range(10):
            ...
            for ind, nr_train_trial in enumerate(range(10, 120, 10)):  # range(120, 120, 10)
                ...
                acc_mat[nr_subj, ind, ind_comp] = acc
                print('S_{0}, #Trial_{1}, Accuracy: {2}'.format(str(nr_subj+1), str(nr_train_trial), str(acc)))
    import pdb;pdb.set_trace()
    with open('TSSF_MTL_acc_{0}.pkl'.format(feat_type), 'wb') as f:
