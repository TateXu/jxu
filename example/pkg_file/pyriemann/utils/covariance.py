...
def _oas(X):
    ...
    if numpy.isrealobj(C):
        return C
    else:
        import yaml
        import os
        if os.path.exists('part.yml'):
            with open('part.yml') as in_part:
                part = yaml.load(in_part)
            if part == 'real':
                return numpy.real(C)
            elif part == 'imag':
                return numpy.imag(C)
                ...
                raise ValueError("Unrecoganized part!")
        else:
            return C
...
def covariances(X, estimator='cov'):
    ...
    if numpy.isrealobj(est(X[0, :, :])):
        covmats = numpy.zeros((Nt, Ne, Ne))
    else:
        covmats = numpy.zeros((Nt, Ne, Ne), dtype=numpy.complex_)
