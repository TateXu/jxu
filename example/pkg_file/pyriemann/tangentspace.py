...
class TangentSpace(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if self.metric == 'logeuclid':
            return tangent_space(X, numpy.eye(Cr.shape[0]))
        else:
    ...
    def fit_transform(self, X, y=None, sample_weight=None):
        ...
        self.reference_ = mean_covariance(X, metric=self.metric,
                                          sample_weight=sample_weight)
        if self.metric == 'logeuclid':
            return tangent_space(X, numpy.eye(self.reference_.shape[0]))
        else:
    ...
    def inverse_transform(self, X, y=None):
        ...
        if self.metric == 'logeuclid':
            return untangent_space(X, numpy.eye(self.reference_.shape[0]))
        else:
